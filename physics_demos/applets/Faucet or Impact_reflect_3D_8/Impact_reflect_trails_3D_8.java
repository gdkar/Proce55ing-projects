import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class Impact_reflect_trails_3D_8 extends PApplet {dot[] dots = new dot[0];
pad[] pads = new pad[0];
public void setup(){
  dots=new dot[400];
  pads=new pad[12];
  ...
  for(int i = 0; i < (pads.length-2); i++){
    pads[i]=new pad(width/2+width/2*cos(PI/(pads.length-2)*(i-1)),height/2+height/2*sin(PI/(pads.length-2)*(i-1)),-width/2,40,2*PI*width/2/(pads.length-2),width*2,0,0,PI/(pads.length-2)*(i-1),.75f);
  }
  pads[pads.length-2]=new pad(width/2,height,width-width/2,width,height/2,40,0,0,0,1);
    pads[pads.length-1]=new pad(width/2,height*.75f,-width-width/2,width,height/2,40,0,0,0,1);
  float[] vert;
  for(int i = 0; i < 1; i++){
    for(int j = 0; j <80; j++){
      for(int k = 0; k < 5; k++){
        dots[i*500+j*5+k] = new dot(30,-j*20,-width/2-(k-2.5f)*20+10*(j%2),0,0,0,10,1,.5f,color(255,(k)*255/5,0));
      }
    }
...
public void draw(){
  ...
  if(keysTapped[ENTER]){
    spawnDot(width/2,height/2,width/2,20*((mouseX-width/2+.1f)/(width/2)),20*((mouseY-height/2+.1f)/(width/2)),-20,10,500,1,color(0,255,0));
  }
  //float eK = 0;
  //for(int i = 0; i < dots.length; i++){
  ...
  for(int i = 0; i < dots.length; i++){
    for(int j = 0; j < pads.length; j++){
      if(cubeSphere(dots[i].x,dots[i].y,dots[i].z,dots[i].r,pads[j].x,pads[j].y,pads[j].z,pads[j].xExt,pads[j].yExt,pads[j].zExt,pads[j].xr,pads[j].yr,pads[j].zr)){
        float[][] vector1 = dCubeSphere(dots[i].x,dots[i].y,dots[i].z,dots[i].r,pads[j].x,pads[j].y,pads[j].z,pads[j].xExt,pads[j].yExt,pads[j].zExt,pads[j].xr,pads[j].yr,pads[j].zr);
        M.MRot(0,-atan2(vector1[0][2],vector1[0][0]),0);
        tM.MRot(0,0,-atan2(vector1[0][1],mag(vector1[0][0],vector1[0][2])));
        M=MMulti(tM,M);
        unM.MRot(0,atan2(vector1[0][2],vector1[0][0]),0);
        tM.MRot(0,0,atan2(vector1[0][1],mag(vector1[0][0],vector1[0][2])));
        unM=MMulti(unM,tM);
        tv[0]=M.MMulti(dots[i].xv,dots[i].yv,dots[i].zv);
        tv[1]=M.MMulti(vector1[0][0],vector1[0][1],vector1[0][2]);
        dots[i].x+=vector1[1][0];
        dots[i].y+=vector1[1][1];
        dots[i].z+=vector1[1][2];
        if(tv[0][0]*tv[1][0]<0){
          float[]vector = unM.MMulti(-tv[0][0]*dots[i].k*pads[j].k,tv[0][1],tv[0][2]);
          dots[i].xv=vector[0];
          dots[i].yv=vector[1];
          dots[i].zv=vector[2];
        }
...
static public void main(String args[]) {   PApplet.main(new String[] { "Impact_reflect_trails_3D_8" });}}