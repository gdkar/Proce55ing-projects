...
void setup(){
  ...
  for(int i = 0; i < (pads.length-2); i++){
    pads[i]=new pad(width/2+width/2*cos(PI/(pads.length-2)*(i-1)),height/2+height/2*sin(PI/(pads.length-2)*(i-1)),-width/2,40,2*PI*width/2/(pads.length-2),width*2,0,0,PI/(pads.length-2)*(i-1),.75);
  }
  pads[pads.length-2]=new pad(width/2,height,width-width/2,width,height/2,40,0,0,0,1);
    pads[pads.length-1]=new pad(width/2,height*.75,-width-width/2,width,height/2,40,0,0,0,1);
  float[] vert;
  for(int i = 0; i < 1; i++){
    for(int j = 0; j <80; j++){
      for(int k = 0; k < 5; k++){
        dots[i*500+j*5+k] = new dot(30,-j*20,-width/2-(k-2.5)*20+10*(j%2),0,0,0,10,1,.5,color(255,(k)*255/5,0));
      }
    }
