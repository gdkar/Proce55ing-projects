import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class Impact_reflect_trails_3D_7 extends PApplet {dot[] dots = new dot[0];
pad[] pads = new pad[0];
PFont font;
public void setup(){
  ...
  M4 M = new M4();
  pads[0]=new pad(width*.125f,height-width/8*sqrt(2),-width/2,width/3*sqrt(2),40,width,0,0,PI/4,.9f);
  pads[1]=new pad(width-(width*.125f),height-width/8*sqrt(2),-width/2,width/3*sqrt(2),40,width,0,0,-PI/4,.9f);
  pads[2]= new pad(width/2,height,-width/2,width/2,40,width,0,0,0,.9f);
  pads[3]=new pad(0,height*3/8,-width/2,40,height*6/8,width,0,0,0,.9f);
  float[] vert;
  for(int i = 0; i < 10; i++){
    for(int j = 0; j <10; j++){
      for(int k = 0; k < 5; k++){
        dots[i*50+j*5+k] = new dot(width/2+(i-5)*20,j*20,-width/2-(k-2.5f)*20+10*(j%2),0,0,0,10,1,1,color(255,(k)*255/5,0));
      }
    }
...
static public void main(String args[]) {   PApplet.main(new String[] { "Impact_reflect_trails_3D_7" });}}